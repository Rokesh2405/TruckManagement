...
$getdetails = FETCH_all("SELECT * FROM `import_tbl` WHERE id=?", $_REQUEST['id']);
$customerdetails = FETCH_all("SELECT * FROM `customer` WHERE customer_name=?", $getdetails['customer_name']);


$stmt = $db->prepare("SELECT * FROM `import_items` WHERE `import_id`='".$_REQUEST['id']."' ORDER BY `id` DESC ");   
$stmt->execute();
...
$balcamt=$getdetails['total_amount']-$getdetails['payment_amount'];
 $message .='</table>
 <br><br>
...
<td width="30%">&nbsp;</td>
<td align="right">$'.number_format($getdetails['balance_amount'],2).'</td>
</tr>
</table>
</td>
</tr>
