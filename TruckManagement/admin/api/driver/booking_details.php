...
$stmt = $db->prepare("SELECT * FROM `booking` WHERE `driver_id`='".$chkrow['id']."' ORDER BY `id` DESC ");	
...
extract($row);
    ...
    $ps_item1[]=array(
            "booking_id"=>$id,
			"from"=>nulltoempty($from_city),
			"to"=>nulltoempty($to_city),
			"date" => date('d-m-Y',strtotime($date)),
			"time"=>date('h:i a',strtotime($date)),
			"km"=>nulltoempty($Distance),
 ...
 if(count($ps_item1)>0) { 
  ...
  echo json_encode(
        array("success" => "true","error" => "false","booking_details" => $ps_item1)
    );
   }
   else
   {
