...
if(
 ...
 $checkvaliduser = $db->prepare("SELECT * FROM `driver` WHERE `username`='".$data->username."' AND `password`='".$data->password."' ORDER BY `id` ASC");
...
$query1 = "UPDATE `driver` SET `device_key`='".$data->device_key."' WHERE `id`='".$row['id']."' ";
...
$stmt1->execute();
  ...
  http_response_code(200);
        ...
        // tell the user
 echo json_encode(array("success" => "true", "error" => "false","driverid" => $row['id'],"token" => $row['token'],"message" => "Login Successfully"));     
 }
 else{
	  http_response_code(200);
