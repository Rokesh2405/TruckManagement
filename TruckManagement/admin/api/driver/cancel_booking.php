...
if(!empty($data->booking_id)) {	
 ...
 $query = "UPDATE `booking` SET
                    status='Cancelled' WHERE id='".$data->booking_id."'";
$stmt = $db->prepare($query);
$stmt->execute();


 $query1 = "INSERT INTO `cancel_trip` (`driver_id`,`booking_id`) VALUES ('".$chkrow['id']."','".$data->booking_id."')";
$stmt1 = $db->prepare($query1);
$stmt1->execute();

   http_response_code(200);
    echo json_encode(
        array("success" => "true","error" => "false","message"=>"Booking Cancelled Successfully")
    );   
