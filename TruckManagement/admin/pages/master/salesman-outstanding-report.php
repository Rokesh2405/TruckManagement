...
$url=$sitename.'MPDF/salesman-outstanding-report.php?type=carrier&fromdate='.$_REQUEST['fromdate'].'&todate='.$_REQUEST['todate'].'&salesman='.$_REQUEST['salesman'];
                ...
                <div class="col-sm-12">
                  ...
                  <h4 class="page-title">Reports</h4>
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active"> Salesman Outstanding Report</li>
                    </ol>
                </div>
</div>
                <!-- /.box-header -->
...
$s1[]="(date(A.`salesman_payment_date`)>='".date('Y-m-d',strtotime($_REQUEST['fromdate']))."'  AND date(A.`salesman_payment_date`)<='".date('Y-m-d',strtotime($_REQUEST['todate']))."')";
}
...
$message1 = $db->prepare("SELECT A.`salesman_payment_date`,A.`load_no`,A.`date`,A.`salesman`,A.`salesman_commission`,B.`payment_amount` FROM `carrier_confirmation` A, `salesman_settlement` B WHERE A.`id`!='0' ORDER BY A.`date` DESC");  
}
