...
function addcustomer1($customer_code,$customer_code1,$customer_code2,$customer_name,$contact_name,$customer_address,$customer_address1,$posta_zip_code,$country,$telephone_admin,$telephone_port,$fax1,$fax2,$getid) {

    global $db;

    if ($id == '') {

        $ress1 = $db->prepare("SELECT `customer_name` FROM `customer` WHERE `id`=?");

        $ress1->execute(array($getid));
        ...
        if ($ress['customer_name'] == '') {
            ...
            $resa = $db->prepare("INSERT INTO `customer` ( `customer_code`,`customer_code1`, `customer_code2`, `customer_name`, `contact_name`, `customer_address`, `customer_address1`, `posta_zip_code`, `country`, `telephone_admin`, `telephone_port`, `fax1`, `fax2`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");

            $resa->execute(array($customer_code,$customer_code1,$customer_code2,$customer_name,$contact_name,$customer_address,$customer_address1,$posta_zip_code,$country,$telephone_admin,$telephone_port,$fax1,$fax2));
        ...
        if ($ress['CusID'] == '') {
            ...
            $resa = $db->prepare("UPDATE `customer` SET `customer_code`=?,`customer_code1`=?, `customer_code2`=?, `customer_name`=?, `contact_name`=?, `customer_address`=?, `City`=?, `Postcode`=?, `Country`=?, `E-mail`=?, `Website`=?, `Mobile`=?, `Phone`=?, `customer_address`=?, `posta_zip_code`=?, `country`=?,`telephone_admin`=?,`telephone_port`=?, `fax1`=?, `fax2`=? WHERE `id`=? ");
